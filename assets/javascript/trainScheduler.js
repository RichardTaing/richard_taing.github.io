...
var firebaseConfig = {
  ...
  databaseURL: "https://train-scheduler-basic-dffe3.firebaseio.com",
  projectId: "train-scheduler-basic-dffe3",
  storageBucket: "",
  messagingSenderId: "299882021658",
  appId: "1:299882021658:web:389260146aa93f3f"
};
// Initialize Firebase
...
$("#submit-btn").on("click", function(event) {
  ...
  var gameSound = new Audio(
    "/assets/sounds/steam-train-whistle-daniel_simon.mp3"
  );
...
function getMinutesAway(time) {
  ...
  var minutesAway = Math.round(
    getNextArrival(time).diff(moment(), "minutes", true)
  );
...
function displayTrainSchedule() {
  ...
  if (firebaseDataObject !== null) {
    Object.keys(firebaseDataObject).forEach(function(key) {
      ...
      newDiv.html("<i class='far fa-edit pop'></i>");
      ...
      newDiv.html("<i class='far fa-trash-alt pop'></i>");
      newTableRow.append($("<td>").html(newDiv));
...
function displayUpdate() {
  ...
  $("#time").val(
    moment(firebaseDataObject[updateKey].firstTrainTime).format("HH:mm")
  );
...
function getInputValues() {
  ...
  time = $("#time")
    .val()
    .trim()
    .replace(/\s/g, ""); //uses regexp removes all white spaces
  frequency = parseInt(
    $("#frequency")
      .val()
      .trim()
